...
- `eus_vessel_features`: 只包含不接触首末行或首末列的完整二维连通域。
- `eus_vessel_boundary_png`: 三类边界共同叠加在一张白底 RGB 图，区域内部保持白色。
- `ct_eus_vessel_overlay_png`: 同一组边界叠加在 CT 灰度图。

完整连通域特征继续使用方形局部毫米坐标：列方向为 `x_mm`，行方向为 `y_mm`，原点与现有特征相同。质心由连通域像素中心计算；面积为像素数乘以两个方向的像素间距。触边连通域仍绘制边界并进入 `eus_vessel_labels`，但不进入 `eus_vessel_features`。
...
- 三类 EUS 血管颜色、合并边界、触边排除特征、毫米质心与面积；
- 原血管特征、两张原图和状态路由完全不变；
- 60% 恰好保留、超过 60% 拒绝；
